...
class PolyspaceApp:
    ...
    def _build_gui(self):
        ...
        # + Button (Top Left)
        add_button = tk.Button(main_frame, text="+", command=self._add_row)
        add_button.grid(row=0, column=0, sticky="nw", padx=5, pady=5)

        # Frame for Rows
        self.rows_frame = tk.Frame(main_frame)
        ...
        run_button = tk.Button(main_frame, text="Run", command=self._run_justifier)
    ...
    def _add_row(self):
        # Separator Line
        separator = tk.Frame(self.rows_frame, height=2, bg="lightgray")
        ...
        # Old CSV Line
        old_frame = tk.Frame(row_frame)
        old_frame.pack(fill="x", padx=30, pady=2)

        old_label = tk.Label(old_frame, text="Old CSV:")
        old_label.pack(side=tk.LEFT, padx=5)
        old_browse_button = tk.Button(old_frame, text="Browse",
                                       command=lambda rf=old_frame, csv_type="old": self._browse_sheet(rf, csv_type))
        old_browse_button.pack(side=tk.LEFT, padx=5)
        old_sheet_var = tk.StringVar()
        ...
        old_sheet_entry = tk.Entry(old_frame, textvariable=old_sheet_var, width=40)
        old_sheet_entry.pack(side=tk.LEFT, fill="x", expand=True, padx=5)

        # Add Input Textbox to Old CSV Line
        input_label_old = tk.Label(old_frame, text="Reference attributes")
        input_label_old.pack(side=tk.LEFT, padx=5)
        input_var_old = tk.StringVar()
        input_entry_old = tk.Entry(old_frame, textvariable=input_var_old, width=50)  # Increased width here
        input_entry_old.pack(side=tk.LEFT, fill="x", expand=True, padx=5)
        input_entry_old.config(width=50)

        # New CSV Line
        new_frame = tk.Frame(row_frame)
        new_frame.pack(fill="x", padx=30, pady=2)

        new_label = tk.Label(new_frame, text="New CSV:")
        new_label.pack(side=tk.LEFT, padx=5)
        new_browse_button = tk.Button(new_frame, text="Browse",
                                       command=lambda rf=new_frame, csv_type="new": self._browse_sheet(rf, csv_type))
        new_browse_button.pack(side=tk.LEFT, padx=5)
        ...
        new_sheet_entry = tk.Entry(new_frame, textvariable=new_sheet_var, width=40)
        new_sheet_entry.pack(side=tk.LEFT, fill="x", expand=True, padx=5)

        # Add Input Textbox
        input_label = tk.Label(new_frame, text="Updated attributes")
        input_label.pack(side=tk.LEFT, padx=5)
        input_var = tk.StringVar()
        input_entry = tk.Entry(new_frame, textvariable=input_var, width=50)  # Increased width here
        input_entry.pack(side=tk.LEFT, fill="x", expand=True, padx=5)
        input_entry.config(width=50)

        # Run Button for this Row
        run_row_button = tk.Button(row_frame, text="Run Row",
                                        command=lambda rf=row_frame: self._run_justifier_for_row(rf))
        run_row_button.pack(side=tk.RIGHT, padx=5)
    ...
    def _browse_sheet(self, row_frame, csv_type):
        ...
        sheet_name = tk.StringVar(value=sheet_names[0])  # Default to first sheet

        popup = tk.Toplevel(self.root)
        popup.title(f"Select {csv_type.capitalize()} CSV Sheet")

        label = tk.Label(popup, text=f"Select {csv_type.capitalize()} CSV Sheet:")
        label.pack(pady=5)
        
        sheet_dropdown = tk.OptionMenu(popup, sheet_name, *sheet_names)
        ...
        select_button = tk.Button(popup, text="Select", command=set_sheet)
